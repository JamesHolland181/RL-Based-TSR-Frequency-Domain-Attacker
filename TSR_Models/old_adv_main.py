...
class AdversarialMaskEnv:
    ...
    def step(self, action):
        ...
        if radius > 1:
            ...
            noise = np.random.uniform(-1, 1, modified.shape).astype(np.float32) * intensity
...
def run_adversarial_testing(images, model_name,
                            ...
                            max_steps=150,
                            save_outputs=False):
    print(f"\n🚀 Testing config: radius={max_patch_radius:.3f}, noise={max_noise:.3f}, steps={max_steps}")
    results_dir = os.path.join(RESULTS_BASE_DIR, model_name)
    os.makedirs(results_dir, exist_ok=True)

    cumulative_reward = 0

    for sample_id, image, _, filename in tqdm(images, desc="🎯 Attacking images"):
        ...
        if save_outputs:
            ...
            for i, frame in enumerate(best_observations):
                plt.imsave(os.path.join(out_dir, f"frame_{i:02d}.png"), frame)
...
def optimize_patch_parameters(images, model_name, n_trials=10):
    def objective(trial):
        return run_adversarial_testing(
            images=images,
    ...
    print("\n🖼 Re-running best trial with image saving...")
